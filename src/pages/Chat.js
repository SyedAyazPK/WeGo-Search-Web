...
const Chat = ({ socket }) => {
  ...
  const item = useSelector(selectRemoteUser);
  ...
  useEffect(() => {
    console.log('This Effect Called');
    if (user?.role == 'user') {
      setRoomId(item?._id);
    } else {
      setRoomId(user?._id);
  ...
  function handleOnEnter(text) {
    if (roomId?.length === 0 || text.length === 0) return;
  ...
  return (
    <div className='grid grid-cols-12 min-h-[70vh]  '>
      ...
      <div className='col-span-12  '>
        ...
        <div className='h-[65vh] py-4  overflow-y-scroll w-full '>
          {msgs?.map((item, i) => (
            <div
              className={`${
                item.fromSelf == true
            ...
            >
              ...
              <p
                className={`${
                  item?.fromSelf === true
                    ? ' bg-[#F6F6F6] text-gray-800  rounded-br-3xl rounded-bl-3xl rounded-tr-3xl '
        ...
        <div className='flex items-center'>
          ...
          />
          <IconButton onClick={() => handleOnEnter(text)}>
            <SendOutlined />
          </IconButton>
        </div>
      </div>
